      ...
      for (file. in list.files(dir.)){
        if (grepl("^A_TPM", file.)){
          ...
          if (mutual_info){
            ...
            saving.dir <- "../../results/networks/big/log_transformed/mutual_info/"
            tryCatch({write.csv(sim.matrix, paste0(saving.dir,"sim_matrix"))},
                 error = function(e) {create.dir(saving.dir)
                   write.csv(sim.matrix, paste0(saving.dir,"sim_matrix"))})
            View(sim.matrix)
          ...
          # High.corr.genes <- get.highCorr.genes(cor.matrix = sim.matrix, threshold = 0.7)
          # # High.corr.genes <- getHighcorGenes(expression.data, 0.5)
...
for (tao in TAO){
  ...
  {  
    ...
    # ======== Save them ========
    if (!mutual_info){
        saving.dir <- sprintf("../../results/networks/big/log_transformed/corr/%s/clusters/", tao)
        ...
        saving.dir <- sprintf("../../results/networks/big/log_transformed/mutual_info/%s/clusters/", tao)
        tryCatch({write.csv(data.frame(red_info[["log"]][[as.character(tao)]][["Clusters"]]$membership), paste0(saving.dir,"clusters.csv"))},
                 error = function(e) {create.dir(saving.dir)
                   write.csv(data.frame(red_info[["log"]][[as.character(tao)]][["Clusters"]]$membership), paste0(saving.dir,"clusters.csv"))})
      }
      ...
      red_info[["log"]][[as.character(tao)]][["Diameter"]] <- diameter(A)
       ...
       #============= SAVE GRAPH
        if (mutual_info){
          saving.dir <- sprintf("../../results/networks/big/log_transformed/mutual_info/%s/graph/", tao)
          tryCatch({write.graph(A, paste0(saving.dir, "graph.txt"),format="ncol")},
               error = function(e) {create.dir(saving.dir)
                 write.graph(A, paste0(saving.dir, "graph.txt"),format="ncol")})
          
        } else if (!mutual_info) {
           saving.dir <- sprintf("../../results/networks/big/log_transformed/corr/%s/graph/", tao)
          tryCatch({write.graph(A, paste0(saving.dir, "graph.txt"),format="ncol")},
               error = function(e) {create.dir(saving.dir)
                 write.graph(A, paste0(saving.dir, "graph.txt"),format="ncol")})
        }
        ...
        if (mutual_info){
          saving.dir <- sprintf("../../results/networks/big/log_transformed/mutual_info/%s/degree/", tao)
          ...
          saving.dir <- sprintf("../../results/networks/big/log_transformed/corr/%s/degree/", tao)
          tryCatch({write.csv(degree.table, paste0(saving.dir, "degree.csv"))},
               error = function(e) {create.dir(saving.dir)
                 write.csv(degree.table, paste0(saving.dir, "degree.csv"))})
        }
    ...
    # ======== Save them ========
    if (!mutual_info){
        saving.dir <- sprintf("../../results/networks/big/raw/corr/%s/clusters/", tao)
        ...
        saving.dir <- sprintf("../../results/networks/big/raw/mutual_info/%s/clusters", tao)
        tryCatch({write.csv(data.frame(red_info[["raw"]][[as.character(tao)]][["Clusters"]]$membership), paste0(saving.dir,"clusters.csv"))},
                 error = function(e) {create.dir(saving.dir)
                   write.csv(data.frame(red_info[["raw"]][[as.character(tao)]][["Clusters"]]$membership), paste0(saving.dir,"clusters.csv"))})
      }
      ...
      #============= SAVE GRAPH
        if (mutual_info){
          saving.dir <- sprintf("../../results/networks/big/raw/mutual_info/%s/graph/", tao)
          tryCatch({write.graph(A, paste0(saving.dir, "graph.txt"),format="ncol")},
               error = function(e) {create.dir(saving.dir)
                 write.graph(A, paste0(saving.dir, "graph.txt"),format="ncol")})
          
        } else if (!mutual_info) {
           saving.dir <- sprintf("../../results/networks/big/raw/corr/%s/graph/", tao)
          tryCatch({write.graph(A, paste0(saving.dir, "graph.txt"),format="ncol")},
               error = function(e) {create.dir(saving.dir)
                 write.graph(A, paste0(saving.dir, "graph.txt"),format="ncol")})
        }
        ...
        if (mutual_info){
          saving.dir <- sprintf("../../results/networks/big/raw/mutual_info/%s/degree/", tao)
          tryCatch({write.csv(degree.table, paste0(saving.dir, "degree.csv"))},
               error = function(e) {create.dir(saving.dir)
                 write.csv(degree.table, paste0(saving.dir, "degree.csv"))})
          ...
          saving.dir <- sprintf("../../results/networks/big/raw/corr/%s/degree/", tao)
          tryCatch({write.csv(degree.table, paste0(saving.dir, "degree.csv"))},
               error = function(e) {create.dir(saving.dir)
                 write.csv(degree.table, paste0(saving.dir, "degree.csv"))})
        }
