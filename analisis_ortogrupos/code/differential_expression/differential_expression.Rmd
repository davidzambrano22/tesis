    ...
    if (grepl("denovo$", dir.)){
      ...
      for (file. in list.files(dir.)){
        ...
        if (grepl("^A_TPM", file.)){
          ...
          #stop("Stopping to determine values of anthocyanin")
          for (a in names(accesions.by.anthocyanin)){
            ...
            for (q in quantiles){
              ...
              if (is.null(out.factors)){
                ...
                resultsNames(dds0)
                # plotDispEsts(dds0)
                print(resultsNames(dds0))
                resLFC0 <- DESeq2::lfcShrink(dds0, coef="contrast_1_vs_0", type="apeglm")
                g_index <- as.numeric(rownames(expresion.data[replace_na(resLFC0$padj < 0.05, FALSE),]))
                DEGS <- c(DEGS, expresion.data$Row.Labels[as.numeric(rownames(expresion.data)) %in% g_index])
                de.novo.DEGS <- c(de.novo.DEGS, DEGS)
                
                print(sprintf("DEGS = %1$s", DEGS))
                print(sprintf("No DEGS is == > %s", length(DEGS)))
                print(cat("Longitud del vector DEGS ==> ", length(DEGS)))
                
              } else {
                print(sprintf("Removing %s from expression data after Size factors calculation", out.factors))
                # #Create clade vector
                # accessions.contrast <- accessions.contrast[accessions.contrast != out.factors]
                # clade.vector <- get.clade.vector(dataset = accesions.info,
                #                                contrast = accessions.contrast,
                #                                mode = "clade")
                # print(cat("Length of clade vector is ==>", length(clado.vector)))
                # #Create constrast factor
                # below_quantile <- c(below_quantile[below_quantile !=  out.factors])
                # print(sprintf("Below quantile length--> %s", length(below_quantile)))
                # above_quantile <- c(above_quantile[above_quantile != out.factors])
                # print(sprintf("Above quantile length--> %s", length(above_quantile)))
                # contrast.vector <- get.contrast.vector(accessions.contrast,
                #                                      below_quantile = below_quantile,
                #                                      above_quantile = above_quantile)
                ...
                message(sprintf("New Deseq object dimensions are ==> %s", dim(dds0)))
                ...
                g_index <- as.numeric(rownames(expresion.data[replace_na(res0$padj < 0.05, FALSE),]))
                DEGS <- c(DEGS, expresion.data$Row.Labels[as.numeric(rownames(expresion.data)) %in% g_index])
                de.novo.DEGS <- c(de.novo.DEGS, DEGS)
         ...
         write.csv(data.frame(table(de.novo.DEGS)),"../../results/DEGS/DEGS_denovo.csv") 
        } 
        }
